...
const findOne = async (req,res,next) => {
    const result = await Content.findOne({where: {id: req.query.id}});
...
const update = async (req,res,next) => {
    ...
    const result = await Content.update(data, {where: {id: req.query.id}});
    console.log(result)
    result.length == 1 
...
const remove = async (req,res,next) => {
    const result = await Content.destroy({where:{id: req.query.id}});
    result 
